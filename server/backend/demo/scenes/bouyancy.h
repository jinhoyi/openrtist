...
public:
	...
	{	
		...
		for (int j=0; j < 1; ++j)
			for (int i=0; i < n; ++i)
				CreateParticleShape(GetFilePathByPlatform("backend/data/sphere.ply").c_str(), Vec3(spacing - 0.5f*size.x + i*spacing * 2, 2.0f + j*size.y*1.2f, 0.6f), size, 0.0f, sampling, Vec3(0.0f, 0.0f, 0.0f), mass[i], true, 1.0f, NvFlexMakePhase(group++, 0), true, 0.0001f);
